...
# process prio raid
  ...
  if [[ $scanCount == 0 ]] ;then
    ...
    scanMin=$(zgrep 'RAIDWATCHER: Raid at gym.*pokemon discovered egg popped' $plog | awk '{print $18}' | sed 's/\[//g' | sed 's/\]//g' | sed 's/\(m[0-9]\)/m\1/g' | awk -F"mm" '{ if (substr($NF,length($NF)-1) == "ms") {print substr($NF,1,length($NF)-2)} else if (substr($NF,length($NF)-1) == "µs") {print substr($NF,1,length($NF)-2) /1000} else if ($1 != $NF) {print $1*60000 + $NF*1000} else {print substr($NF,1,length($NF)-1) *1000} }' | jq -s min)
    scanMax=$(zgrep 'RAIDWATCHER: Raid at gym.*pokemon discovered egg popped' $plog | awk '{print $18}' | sed 's/\[//g' | sed 's/\]//g' | sed 's/\(m[0-9]\)/m\1/g' | awk -F"mm" '{ if (substr($NF,length($NF)-1) == "ms") {print substr($NF,1,length($NF)-2)} else if (substr($NF,length($NF)-1) == "µs") {print substr($NF,1,length($NF)-2) /1000} else if ($1 != $NF) {print $1*60000 + $NF*1000} else {print substr($NF,1,length($NF)-1) *1000} }' | jq -s max)
    scanAvg=$(zgrep 'RAIDWATCHER: Raid at gym.*pokemon discovered egg popped' $plog | awk '{print $18}' | sed 's/\[//g' | sed 's/\]//g' | sed 's/\(m[0-9]\)/m\1/g' | awk -F"mm" '{ if (substr($NF,length($NF)-1) == "ms") {print substr($NF,1,length($NF)-2)} else if (substr($NF,length($NF)-1) == "µs") {print substr($NF,1,length($NF)-2) /1000} else if ($1 != $NF) {print $1*60000 + $NF*1000} else {print substr($NF,1,length($NF)-1) *1000} }' | jq -s add/length)
    raidMin=$(zgrep 'RAIDWATCHER: Raid at gym.*pokemon discovered egg popped' $plog | awk '{print $25}' | sed 's/\[//g' | sed 's/\]//g' | sed 's/\(m[0-9]\)/m\1/g' | awk -F"mm" '{ if (substr($NF,length($NF)-1) == "ms") {print substr($NF,1,length($NF)-2)} else if (substr($NF,length($NF)-1) == "µs") {print substr($NF,1,length($NF)-2) /1000} else if ($1 != $NF) {print $1*60000 + $NF*1000} else {print substr($NF,1,length($NF)-1) *1000} }' | jq -s min)
    raidMax=$(zgrep 'RAIDWATCHER: Raid at gym.*pokemon discovered egg popped' $plog | awk '{print $25}' | sed 's/\[//g' | sed 's/\]//g' | sed 's/\(m[0-9]\)/m\1/g' | awk -F"mm" '{ if (substr($NF,length($NF)-1) == "ms") {print substr($NF,1,length($NF)-2)} else if (substr($NF,length($NF)-1) == "µs") {print substr($NF,1,length($NF)-2) /1000} else if ($1 != $NF) {print $1*60000 + $NF*1000} else {print substr($NF,1,length($NF)-1) *1000} }' | jq -s max)
    raidAvg=$(zgrep 'RAIDWATCHER: Raid at gym.*pokemon discovered egg popped' $plog | awk '{print $25}' | sed 's/\[//g' | sed 's/\]//g' | sed 's/\(m[0-9]\)/m\1/g' | awk -F"mm" '{ if (substr($NF,length($NF)-1) == "ms") {print substr($NF,1,length($NF)-2)} else if (substr($NF,length($NF)-1) == "µs") {print substr($NF,1,length($NF)-2) /1000} else if ($1 != $NF) {print $1*60000 + $NF*1000} else {print substr($NF,1,length($NF)-1) *1000} }' | jq -s add/length)
  fi
